module Standings
  class RankEvaluator
    ...
    def_delegator :@config, :scope, :scope
    ...
    def top_rankers(user_limit)
      scope.order("#{column_name} DESC, #{sort_order}")
           .limit(user_limit)
    end
    ...
    def high_rankers
      rankers = scope.where("#{column_name} > ?", column_value)
    ...
    def equal_rankers
      @equal_rankers ||= scope.where("#{column_name} = ?", column_value)
                              .order(sort_order)
                              .select(:id)
    end
